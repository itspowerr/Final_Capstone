...
async def google_login(request: GoogleAuthRequest, db: AsyncSession = Depends(get_db)):
    if not settings.google_client_id:
        raise HTTPException(
            ...
            detail={"code": "GOOGLE_AUTH_NOT_CONFIGURED", "message": "Google login is not configured."},
        )
    ...
    if role not in ("client", "freelancer"):
        raise HTTPException(
            ...
            detail={"code": "INVALID_ROLE", "message": "Google login role must be client or freelancer."},
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail={"code": "INVALID_GOOGLE_TOKEN", "message": "Google credential could not be verified."},
        )

    if profile.get("aud") != settings.google_client_id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail={"code": "GOOGLE_AUDIENCE_MISMATCH", "message": "Google credential was issued for another app."},
        )

    if profile.get("email_verified") not in (True, "true", "True"):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail={"code": "GOOGLE_EMAIL_UNVERIFIED", "message": "Google email is not verified."},
        )
    ...
    if not email:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"code": "GOOGLE_EMAIL_MISSING", "message": "Google account did not provide an email address."},
        )
    ...
    if user:
        if user.role.value != role:
            raise HTTPException(
                ...
                detail={
                    "code": "ROLE_MISMATCH",
                    "message": f"This Google account is registered as {user.role.value}.",
                },
            )
