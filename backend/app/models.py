...
class User(Base):
    ...
    role = Column(Enum(UserRole, name="user_role", schema="freeledger"), default=UserRole.freelancer)
...
class Contract(Base):
    ...
    on_chain_id = Column(Integer, nullable=True, unique=True)
    contract_address = Column(String(42), nullable=True)
    status = Column(Enum(ContractStatus, name="contract_status", schema="freeledger"), default=ContractStatus.draft, index=True)
...
class Dispute(Base):
    ...
    status = Column(Enum(DisputeStatus, name="dispute_status", schema="freeledger"), default=DisputeStatus.open, index=True)
    decision = Column(Enum(DisputeDecision, name="dispute_decision", schema="freeledger"), nullable=True)
...
class Thread(Base):
    ...
    __table_args__ = {"schema": "freeledger"}
    id = Column(String(50), primary_key=True, default=lambda: generate_pseudonymous_id("th"))
    client_id = Column(String(50), ForeignKey("freeledger.users.id"), nullable=False, index=True)
    freelancer_id = Column(String(50), ForeignKey("freeledger.users.id"), nullable=False, index=True)
    job_id = Column(String(50), ForeignKey("freeledger.jobs.id"), nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class Message(Base):
    ...
    __table_args__ = {"schema": "freeledger"}
    id = Column(String(50), primary_key=True, default=lambda: generate_pseudonymous_id("msg"))
    sender_id = Column(String(50), ForeignKey("freeledger.users.id"), nullable=False, index=True)
    receiver_id = Column(String(50), ForeignKey("freeledger.users.id"), nullable=False, index=True)
    content = Column(Text, nullable=False)
    read = Column(Boolean, default=False, nullable=False)
    thread_id = Column(String(50), ForeignKey("freeledger.threads.id"), nullable=False, index=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
