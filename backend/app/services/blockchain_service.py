...
def get_milestone_state(on_chain_id: int, milestone_index: int) -> dict:
    contract = get_contract()
    details = contract.functions.getMilestoneDetails(on_chain_id, milestone_index).call()
    return {
        "description": details[0],
        "amount": details[1],
        "deliverable_cid": details[2],
        "status": details[3],
        "submitted_at": details[4],
        "approved_at": details[5],
    }
...
def build_contract_tx(fn, address: str, gas: int | None = None, value: int = 0) -> dict:
    w3 = get_web3()
    checksum_address = Web3.to_checksum_address(address)
    nonce = w3.eth.get_transaction_count(checksum_address)
    gas_price = w3.eth.gas_price
    tx_params = {"from": checksum_address, "nonce": nonce, "gasPrice": gas_price, "value": value}

    if gas is None:
        try:
            estimated = fn.estimate_gas({"from": checksum_address, "value": value})
            tx_params["gas"] = max(estimated + 50000, int(estimated * 1.25))
        except Exception:
...
def sign_and_send(tx: dict, private_key: str) -> str:
    ...
    receipt_hash = getattr(receipt, "transaction_hash", None) or getattr(receipt, "transactionHash")
