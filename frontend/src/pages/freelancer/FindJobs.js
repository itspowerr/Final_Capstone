...
function EmptySearchIcon() {
  return (
    <div className="empty-icon jobs-empty-icon" aria-hidden="true">
...
export default function FindJobs() {
  ...
  async function submitApplication() {
    ...
    if (!applyForm.cover_letter.trim() || Number.isNaN(bid) || bid <= 0) {
      showToast('Please enter a cover letter and valid bid amount.', '!');
      return;
    }
      ...
      <div key={job.id} className={'job-card' + (isApplied ? ' applied' : '') + (currentView === 'list' ? ' list-card' : '')} onClick={() => openModal(job)} style={{ cursor: 'pointer', position: 'relative' }}>
        ...
        <div className="job-footer">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ fontSize: 12, color: 'var(--text-3)' }}>{name || 'Client'}</span>
          </div>
        </div>
      </div>
    );
