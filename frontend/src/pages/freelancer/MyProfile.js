...
export default function MyProfile() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setFullName(data.username || saved.fullName || '');
        setTitle(data.headline || saved.title || '');
        setBio(data.bio || saved.bio || '');
        setLocation(data.location || saved.location || '');
        setGithub(data.github || saved.github || '');
        setPortfolio(data.portfolio || saved.portfolio || '');
        setLinkedin(data.linkedin || saved.linkedin || '');
        setSkills((data.skills && data.skills.length > 0) ? data.skills : (saved.skills || []));
        setHourlyRate(data.hourly_rate || saved.hourlyRate || '');
        setExperience(data.experience_level || saved.experience || 'mid');
        setAvailability(saved.availability || (data.is_available ? 'available' : 'busy'));
        setWallet(data.wallet_address || saved.wallet || '');
      } catch {
        const saved = JSON.parse(localStorage.getItem('fl_freelancer_profile') || '{}');
  ...
  function snapshotProfile(nextWallet = wallet) {
    return {
      fullName, title, bio, location, github, portfolio, linkedin,
      hourlyRate, experience, availability, skills, wallet: nextWallet,
    };
  }

  async function saveProfile() {
    const profileSnapshot = snapshotProfile();
    localStorage.setItem('fl_freelancer_profile', JSON.stringify(profileSnapshot));
    try {
  ...
  async function connectMetaMask() {
    if (typeof window.ethereum !== 'undefined') {
      try {
        ...
        localStorage.setItem('fl_freelancer_profile', JSON.stringify(snapshotProfile(accounts[0])));
