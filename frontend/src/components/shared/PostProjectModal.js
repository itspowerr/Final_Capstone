...
export default function PostProjectModal({ isOpen, onClose }) {
  ...
  const runDiagnostics = () => {
    const diag = {};
    diag.metaMaskInstalled = !!window.ethereum;
    diag.hasAccessToken = !!localStorage.getItem('access_token');
    diag.hasRefreshToken = !!localStorage.getItem('refresh_token');
    try {
      const u = localStorage.getItem('user');
      diag.user = u ? JSON.parse(u) : null;
    } catch { diag.user = null; }
    diag.expectedChainId = '0x' + config.chainId.toString(16);
    diag.expectedRpc = config.rpcUrl;
    if (window.ethereum) {
      window.ethereum.request({ method: 'eth_chainId' })
        .then((id) => { diag.chainId = id; })
        .catch(() => { diag.chainId = 'error'; });
    }
    fetch(config.apiUrl.replace('/api', '') + '/docs', { method: 'HEAD' })
      .then((r) => { diag.backendReachable = r.ok; })
      .catch(() => { diag.backendReachable = false; })
      .finally(() => { /* diagnostics intentionally silent in production */ });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      blockchainStep = 'ensureCorrectNetwork';
      await ensureCorrectNetwork();

      blockchainStep = 'getSigner';
      await getSigner();

      blockchainStep = 'getContract';
      ...
      const reason = chainErr?.reason || '';
      const msg = chainErr?.message || chainErr?.toString?.() || 'Unknown blockchain error';
      console.error(`[PostProjectModal] Blockchain step "${blockchainStep}" failed:`, {
        step: blockchainStep,
        code,
        reason,
      ...
      setPostError(`Blockchain deployment failed at "${blockchainStep}": ${msg}`);
      setPosting(false);
    ...
    try {
      ...
      setPostSuccess('Project posted successfully. Your contract is now linked on-chain.');
      window.setTimeout(() => {
        resetForm();
        onClose();
      }, 900);
      ...
      console.error(`[PostProjectModal] Backend API failed:`, {
    ...
    <div className={`modal-overlay${isOpen ? ' open' : ''}`} onClick={close}>
      <div className="modal-box" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {postSuccess && <div className="form-success-msg" style={{ marginBottom: 8, textAlign: 'center' }}>{postSuccess}</div>}
